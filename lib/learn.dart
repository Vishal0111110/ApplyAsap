...
import 'course_chatbot.dart';
import 'practice_chatbot.dart';
...
class _CareerGeminiPageState extends State<CareerGeminiPage>
  ...
  final TextEditingController _questionCustomController =
  ...
  String _tr(String key, {Map<String, String>? params}) {
    ...
    String translated =
        translations?[widget.lang] ?? translations?["English"] ?? key;
    if (params != null) {
  ...
  Future<void> _initiateLinkGeminiCall({String? customQuery}) async {
    ...
    final queryKey = customQuery ?? 'default';
    ...
    if (snapshot.exists) {
      ...
      freeCourseLinks = List<Map<String, String>>.from(coursesData.map((item) {
        return {
          'title': item['title'] as String,
          'url': item['url'] as String,
        };
      }));
      debugPrint('Fetched courses from DB');
      setState(() {
    ...
    if (freeCourseLinks.isEmpty) {
      freeCourseLinks = [
        {
          'title': _tr("YouTube: Free {career} Tutorials",
              params: {"career": widget.career}),
          'url':
              'https://www.youtube.com/results?search_query=${widget.career}+tutorial+${widget.lang}'
        },
        {
          'title': _tr("Udemy: Free {career} Courses",
              params: {"career": widget.career}),
          'url':
              'https://www.udemy.com/courses/free/?q=${widget.career}+${widget.lang}'
        },
        {
          'title': _tr("Coursera: Free {career} Courses",
              params: {"career": widget.career}),
          'url':
              'https://www.coursera.org/courses?query=${widget.career}+${widget.lang}&price=free'
        },
        {
          'title': _tr("Class Central: {career} Courses",
              params: {"career": widget.career}),
          'url':
              'https://www.classcentral.com/search?q=${widget.career}+${widget.lang}'
        },
        {
          'title': _tr("edX: Free {career} Courses",
              params: {"career": widget.career}),
          'url':
              'https://www.edx.org/learn/${widget.career}?language=${widget.lang}'
        },
      ];
    }
  ...
  Future<void> _loadStaticQuestions(String category) async {
    try {
      String jsonString;
      if (category == "DSA") {
        jsonString =
      ...
      final List<CodingQuestion> loadedQuestions =
        ...
        return CodingQuestion(
          ...
          category: category,
          testCases: (jsonItem['testCases'] as List)
              .map((tc) => TestCase(
                    input: tc['input'],
                    output: tc['output'],
                    hidden: tc['hidden'],
  ...
  Future<void> _loadEnglishQuestions() async {
    ...
    try {
      ...
      List<Question> loadedQuestions = questionsJson.map((jsonItem) {
        return Question(
          questionText: jsonItem['questionText'],
      ...
      setState(() {
        ...
        codingQuestions = []; // Clear coding questions
        _isQuestionsLoading = false;
      });
    } catch (e) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
            content:
                Text('Failed to load English questions. Please try again.')),
      );
      setState(() {
        _isQuestionsLoading = false;
      });
  ...
  Future<void> _initiateQuestionGeminiCall(
      {String? customQuery, bool append = false}) async {
    setState(() {
      _isQuestionsLoading = true;
    });
    ...
    // Load static questions for DSA and Dev
    if (customQuery == "DSA" || customQuery == "Dev") {
      await _loadStaticQuestions(customQuery!);
      return;
    }

    // Check if data exists in DB
    final snapshot = await dbRef.get();
    if (snapshot.exists && !append) {
      final data = snapshot.value as Map;
      ...
      debugPrint('Fetched questions from DB');
      setState(() {
        _isQuestionsLoading = false;
      });
      return;
    ...
    try {
      ...
      if (!append) {
        ...
        // For append, update the existing list
        final existingData = snapshot.exists ? snapshot.value as Map : {};
        final existingQuestions = existingData['questions'] as List? ?? [];
        existingQuestions.addAll(generatedQuestions
            .map((q) => {
                  'questionText': q.questionText,
                  'options': q.options,
                  'correctOptionIndex': q.correctOptionIndex,
      ...
      // Fallback to default MCQs for other categories
      setState(() {
        questions = [
          Question(
            questionText:
                _tr("What does Flutter primarily use for building UIs?"),
            options: [
              _tr("Dart"),
              _tr("JavaScript"),
              _tr("Kotlin"),
          ...
          Question(
            questionText:
                _tr("What programming language is used with Flutter?"),
            options: [_tr("Dart"), _tr("Python"), _tr("Ruby"), _tr("C#")],
            correctOptionIndex: 0,
          ),
        ];
      });
  ...
  Future<void> _saveSubmittedQuestion(
      String questionText, int selectedOption, bool answered) async {
    final String userId = FirebaseAuth.instance.currentUser?.uid ?? "";
  ...
  void _checkIndividualAnswer(int questionIndex) async {
    ...
    _saveSubmittedQuestion(
        question.questionText, question.selectedOptionIndex!, true);
    if (isCorrect) {
      // Award points using gamification service
      GamificationService().awardPointsWithPopup(context,
          FirebaseAuth.instance.currentUser?.uid ?? '', 5, 'Correct Answer!');

      // Update challenge progress for questions completed
      final userId = FirebaseAuth.instance.currentUser?.uid ?? '';
      if (userId.isNotEmpty) {
        await GamificationService()
            .updateChallengeProgress(userId, 'questions_completed', 'weekly');
        await GamificationService()
            .updateChallengeProgress(userId, 'questions_completed', 'monthly');

        // Show success message indicating challenge progress updated
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content:
                Text('Challenge progress updated! Check the Challenges tab.'),
              ...
              Row(
                children: [
                  ...
                  ElevatedButton(
                    ...
                    onPressed: () {
                      _initiateLinkGeminiCall(
                          customQuery: _courseCustomController.text);
                    },
                    child: Text(
                      _tr("Generate"),
                      style: const TextStyle(color: Colors.white, fontSize: 16),
    ...
    return Stack(
      children: [
        Padding(
          ...
          child: Column(
            children: [
              _buildQuestionCategorySelector(),
              const SizedBox(height: 12),
              Expanded(
                    ...
                    : ListView.separated(
                        ...
                        itemBuilder: (context, index) {
                          if (questions.isNotEmpty) {
                            ...
                            final codingQuestion = codingQuestions[index];
              ...
              Row(
                children: [
                  ...
                  ElevatedButton(
                    ...
                    onPressed: () {
                      _initiateQuestionGeminiCall(
                          customQuery: _questionCustomController.text);
                    },
                    child: Text(
                      _tr("Generate"),
                      style: const TextStyle(color: Colors.white, fontSize: 16),
        ...
        // Only show chatbot for Technical, Aptitude, and English categories
        if (_selectedCategory != "DSA" && _selectedCategory != "Dev")
          Positioned(
            bottom: 95,
            right: 16,
            child: FloatingActionButton(
  ...
  Widget _buildQuestionCategorySelector() {
    // Define software-related careers that should show DSA and Dev categories
    ...
    final career = widget.career.toLowerCase();
    final isSoftwareRelated = softwareCareers.any(
        (swCareer) => career.contains(swCareer) || swCareer.contains(career));
    ...
    final categories = [
      ...
      },
      {
        ...
        'handler': () {
          setState(() {
            _selectedCategory = "Aptitude";
          });
          _initiateQuestionGeminiCall(customQuery: "Aptitude");
        }
      },
      {
        'name': 'English',
        ...
        'handler': () {
          setState(() {
            _selectedCategory = "English";
          });
          _loadEnglishQuestions();
        }
      },
      if (isSoftwareRelated) ...[
        {
          ...
          'handler': () {
            setState(() {
              _selectedCategory = "DSA";
            });
            _initiateQuestionGeminiCall(customQuery: "DSA");
          }
        },
        {
          'name': 'Dev',
          ...
          'handler': () {
            setState(() {
              _selectedCategory = "Dev";
            });
            _initiateQuestionGeminiCall(customQuery: "Dev");
          }
        },
      ]
    ];
      ...
      child: Wrap(
        ...
        children: categories.map((category) {
          ...
          return ElevatedButton(
            onPressed: () => (category['handler'] as Function)(),
            style: ElevatedButton.styleFrom(
  ...
  Widget _buildCodingQuestionCard(int questionIndex, CodingQuestion question) {
    // Check if question is completed using Firebase path
    final questionId = question.questionText.hashCode.toString();
    final currentUser = FirebaseAuth.instance.currentUser;
    final completedRef = currentUser != null
    ...
    return StreamBuilder<DatabaseEvent>(
      stream: completedRef?.onValue,
      builder: (context, snapshot) {
        final isCompleted = snapshot.hasData && snapshot.data!.snapshot.exists;
          ...
          child: InkWell(
            onTap: () {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (_) => IDEScreen(question: question),
                ),
              );
            },
            borderRadius: BorderRadius.circular(16),
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  // Title row with question text and status
                  Row(
                    children: [
                      // Code icon indicator
                      Container(
                  ...
                  // Status message (only if completed)
                  if (isCompleted)
                    Container(
  ...
  Widget _buildQuestionCard(int questionIndex, Question question) {
    ...
    if (_submittedQuestions.containsKey(key)) {
      ...
      question.selectedOptionIndex =
          _submittedQuestions[key]['selectedOptionIndex'];
    }
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            // Options grid with improved spacing and styling
            Container(
              ...
              child: Column(
                children: List.generate(question.options.length, (optionIndex) {
                  ...
                  bool isCorrectAnswer = question.answered &&
                      question.correctOptionIndex == optionIndex;
                  bool isIncorrectSelection = question.answered &&
                      isSelected &&
                      question.correctOptionIndex != optionIndex;
                            ...
                            color: isCorrectAnswer
                                ...
                                : isIncorrectSelection
                                    ...
                                    : isSelected
                                        ...
                                        : Colors.transparent,
                            width:
                                isCorrectAnswer || isIncorrectSelection ? 2 : 1,
                          ),
                        ),
                        child: Row(
                          children: [
                            ...
                            // Selection indicator
                            if (question.answered && isSelected)
                              Icon(
                                isCorrectAnswer
                                    ? Icons.check_circle
                                    : Icons.cancel,
