...
class _ChatScreenState extends State<ChatScreen> {
  ...
  String getChatContent() {
    ...
    for (var message in _chatHistory) {
      final sender = message.isUserMessage
          ? (localized["you"] ?? "You")
...
'''
    ...
    try {
      ...
      if (snapshot.exists) {
        ...
        final sessions = data.entries.map((entry) {
          ...
          return ChatSession(
            id: entry.key,
            title: sessionData['title'] ?? 'Career Chat Session',
      ...
      debugPrint('Error loading career chat history: $e');
    }
  }

  Future<void> _saveCurrentChat() async {
    if (_chatHistory.length <= 1) return;
    ...
    final firstUserMessage = _chatHistory.firstWhere(
    ...
    final messagesData = _chatHistory
        .map((msg) => {
              'content': msg.content,
    ...
    try {
      ...
      debugPrint('Error saving career chat: $e');
    }
  }
  ...
  Future<void> _deleteChatSession(String sessionId) async {
    ...
    try {
      ...
      debugPrint('Error deleting career chat session: $e');
    }
  }
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: const Text(
            'Are you sure you want to delete this career chat session?',
                ...
                children: [
                        ...
                        onTap: () {
                          ...
                          Navigator.of(context).push(MaterialPageRoute(
                            builder: (context) => InAppWebViewScreen(
                              url: _overleafUrl!,
                              title: localized["openInOverleaf"] ??
                                  "Open in Overleaf",
                            ),
                          ));
                        },
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      border:
                          Border.all(color: const Color(0xFF424242), width: 1),
                    ),
                    child: Row(
                      children: [
                        Expanded(
                        ...
                        IconButton(
                          onPressed: !_awaitingResponse
                              ...
                              : null,
                          icon:
                              const Icon(Icons.send, color: Color(0xFF5BC0EB)),
                        ),
                      ],
                    ),
                  ),
              ...
              Navigator.of(context).push(
                MaterialPageRoute(
                  builder: (context) => ShareChatScreen(
                    ...
                    chatTitle: 'Career Chat with Friday',
      ...
      body: Column(
        children: [
          ...
          Container(
            padding: const EdgeInsets.all(16),
            child: const Text(
              'Share your career insights with friends and colleagues!',
              style: TextStyle(
                color: Color(0xFFB0B0B0),
                fontSize: 14,
