...
class _FeedPageState extends State<FeedPage> {
  ...
  String _tr(String key, {Map<String, String>? params}) {
    ...
    String translated =
        translations1?[lang] ?? translations1?["English"] ?? key;
    if (params != null) {
                    ...
                    child: ElevatedButton(
                      ...
                      onPressed: () async {
                        ...
                        if (content.isNotEmpty || _mediaBase64 != null) {
                          ...
                          await notificationService.sendFeedMessageNotification(
                              newPostRef.key!,
                              FirebaseAuth.instance.currentUser?.uid ?? "",
  ...
  Future<void> _handleLike(String postId, int currentLikes) async {
    ...
    await GamificationService().awardPointsWithPopup(context,
        FirebaseAuth.instance.currentUser?.uid ?? '', 5, 'Post Liked!');

    // Send notification for new like
    final notificationService = NotificationService();
    // Get post author ID from the post data
    final postSnapshot = await _postsRef.child(postId).get();
    if (postSnapshot.exists) {
      final postData = Map<String, dynamic>.from(postSnapshot.value as Map);
      final postAuthorId = postData['authorId'] ?? postData['author'] ?? '';
                      ...
                      ElevatedButton(
                        ...
                        onPressed: () async {
                          ...
                          if (commentContent.isNotEmpty) {
                            ...
                            final commenterId =
                                FirebaseAuth.instance.currentUser?.uid ?? "";
                            final commenterName = FirebaseAuth
                                    .instance.currentUser?.displayName ??
                                "Unknown User";
  ...
  Widget _buildPostCard(String postId, Map<dynamic, dynamic> postData) {
    ...
    final comments =
        postData['comments'] != null ? (postData['comments'] as Map).length : 0;
                        ...
                        child: Row(
                          ...
                          children: [
                            Icon(
                              ...
                              size: 18,
                              color: _likedPosts.contains(postId)
