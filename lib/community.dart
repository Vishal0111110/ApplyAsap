...
class _CommunityPageState extends State<CommunityPage> {
                    ...
                    child: ElevatedButton(
                      ...
                      onPressed: () async {
                        if (_formKey.currentState!.validate()) {
                          ...
                          // Send notification about new community creation
                          final notificationService = NotificationService();
                          await notificationService
                              .sendCommunityCreationNotification(
                                  name, currentUserId!);

                          // Award points using gamification service
                          final int pointsAwarded = isPublic ? 20 : 10;
  ...
  // Build list view for communities with joined communities at top.
  Widget _buildCommunityList(List<Community> communities) {
    ...
    List<Community> notJoinedCommunities = [];
    ...
    // Combine lists with joined communities first
    ...
    return ListView.builder(
      ...
      itemCount: sortedCommunities.length +
          (joinedCommunities.isNotEmpty && notJoinedCommunities.isNotEmpty
              ? 1
              : 0),
      itemBuilder: (context, index) {
        // Add separator between joined and not joined communities
        if (joinedCommunities.isNotEmpty &&
            notJoinedCommunities.isNotEmpty &&
            index == joinedCommunities.length) {
          return Container(
        ...
        return _buildCommunityCard(sortedCommunities[adjustedIndex]);
      },
    );
  }
